...
class InvoiceService
{
    ...
    {
        $application = Application::with(['vehicle', 'user'])->findOrFail($id);
        $vehicle     = $application->vehicle;

        $financing = \App\Models\Financing::with('services')->where('application_id', $id)->first();

        $html = self::buildHtml($application, $vehicle, $financing);
    ...
    private static function buildHtml($application, $vehicle, $financing): string
    {
        $fecha          = $application->created_at->format('d/m/Y');
        $nombre         = $application->full_name ?? 'N/A';
        $cedula         = $application->cedula    ?? 'N/A';
        $telefono       = $application->phone     ?? 'N/A';
        $direccion      = $application->direccion ?? 'N/A';
        $tipo           = $application->type === 'cash' ? 'De contado' : 'Financiación';
        $plan           = $application->plan      ?? 'N/A';
        ...
        $payInitial   = $financing && $financing->payment_initial ? 'Sí' : 'No';
        ...
        $vehicleRow = '<tr style="background:#ffffff;">'
            . '<td style="padding:10px 14px;border-bottom:1px solid #e8ecf1;"><strong>' . $marca . ' ' . $modelo . '</strong>'
            . '<br><span style="font-size:10px;color:#777;">' . $tipo . ' &mdash; Plan: ' . $plan . '</span></td>'
            . '<td style="padding:10px 14px;border-bottom:1px solid #e8ecf1;text-align:center;">—</td>'
            . '<td style="padding:10px 14px;border-bottom:1px solid #e8ecf1;text-align:center;">1</td>'
        ...
        foreach ($cuotas as $label => $val) {
            if ((float) $val > 0) {
                ...
                $itemRows .= '<tr style="background:' . $bg . ';">'
                    ...
                    . '<td style="padding:10px 14px;border-bottom:1px solid #e8ecf1;text-align:center;">' . $plan . '</td>'
                    . '<td style="padding:10px 14px;border-bottom:1px solid #e8ecf1;text-align:center;">' . $installments . '</td>'
  ...
  <div class="bottom">
    ...
    <div class="bottom-left">
      ...
      <table class="fin-grid">
        <tr><td class="lbl">Tipo de operación</td><td>{$tipo}</td><td class="lbl">Estado de mora</td><td>{$moraStatus}</td></tr>
        <tr><td class="lbl">Duración</td><td>{$months} meses</td><td class="lbl">Pago inicial</td><td>{$payInitial}</td></tr>
        <tr><td class="lbl">N.º de cuotas</td><td>{$installments}</td><td class="lbl">Interés</td><td>{$interesPct}</td></tr>
      </table>
      {$observationSection}
    </div>
