...
<template>
  <div class="px-5">
    <TablesHeader title="Carros" icon="fa-solid fa-car" 
    :searchActive="true" @setSearch="({ e }) => setSearch(e)"
    :btnCreate="true" @create="router.push('/vehicles/cars/create')" />

    <RouterView :conditionForView="$route.path.includes('/vehicles/cars/create') || $route.path.includes('/vehicles/cars/edit')"/>
    ...
    <section class="relative mx-auto mb-4 overflow-auto animate-fade-in">
      <div class="fakeTable md:w-[90%] mx-auto h-[70vh] py-10" @scroll="loadScroll">
        <section class="grid grid-cols-1 gap-5 md:grid-cols-2" v-if="data.rows.length > 0">
          <CardVehicles v-for="row in data.rows" :key="row.id" :vehicle="row" 
